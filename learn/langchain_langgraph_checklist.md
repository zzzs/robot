...
- ✅ `streamMode: 'messages'` 拿 token 级流 — `langgraph-orchestrator.ts` + `supervisor-orchestrator.ts`
- ✅ `subgraphs: true` 让子图 token 事件透传 — `supervisor-orchestrator.ts`
- ✅ `metadata.langgraph_node` 过滤用户可见节点的 token — 两个 orchestrator
...
- ☐ `createAgent`(一行创建 ReAct agent)— ⚠️ `createReactAgent` 已弃用,新 API 是 `createAgent`,从 `langchain` 包导入: `import { createAgent } from 'langchain'`
- ☐ `createAgent` + `stateModifier`(在 LLM 调用前修改 state)
- ⭐ 试一下 `createAgent`,跟手写版对比
...
| ⭐⭐ | **`createAgent` 对比手写**(`createReactAgent` 已弃用,改用 `langchain` 包的 `createAgent`) | 知道什么时候用 prebuilt、什么时候手写 |
