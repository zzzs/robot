...
- ✅ `RecursiveCharacterTextSplitter`(最常用)— `backend/src/news/news-embedding.service.ts`
...
- ✅ `OpenAIEmbeddings` / `CohereEmbeddings`(OpenAIEmbeddings 指向 DashScope 兼容端点)— `backend/src/news/news-embedding.service.ts`
...
- ✅ `MemoryVectorStore`(纯内存,v1 用) — `backend/src/news/news-embedding.service.ts`

### Retrievers

- ✅ `vectorStore.asRetriever()` 基础相似度检索 — `backend/src/news/news-retrieval.service.ts`
...
- ✅ `streamMode: 'messages'` 拿 token 级流 — `chat/langgraph-orchestrator.ts` + `chat/supervisor-orchestrator.ts`
...
| 优先级 | 主题 | 为什么 |
|---|---|---|
| ✅ | **LangGraph token 级流式** (`streamMode: 'messages'`) — 已完成 | 见 `learn/langgraph_react.md` 的 "Token streaming" 一节 |
