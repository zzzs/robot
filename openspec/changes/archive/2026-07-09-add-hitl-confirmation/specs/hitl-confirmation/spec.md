...
系统 SHALL 提供 `POST /api/chat/resume` 端点,接收 `{ sessionId, action: 'confirm' | 'cancel' }`。该端点 SHALL:
- 从 MemorySaver 恢复指定 sessionId 的 checkpoint
- 用 `Command(resume: action === 'confirm' ? 'confirmed' : 'cancelled')` 注入用户响应
- 恢复 graph 执行,以 SSE 流式返回后续结果(跟正常 stream 一致)

#### Scenario: 用户确认后恢复
- **WHEN** `POST /api/chat/resume?sessionId=xxx&action=confirm`
- **THEN** graph 从 interrupt 处恢复
- **AND** confirm 节点收到 `'confirmed'`,继续到 agent
- **AND** 后续 SSE 流正常 emit text(chart 已在 interrupt 前发过)+ done

#### Scenario: 用户取消
- **WHEN** `POST /api/chat/resume?sessionId=xxx&action=cancel`
- **THEN** graph 从 interrupt 处恢复
- **AND** confirm 节点收到 `'cancelled'`,返回"已取消"文本
- **AND** SSE 流 emit text("已取消,未展示分析结果。") + done

#### Scenario: 没有 pending interrupt 时 resume
- **WHEN** `POST /api/chat/resume?sessionId=xxx` 但该 session 没有 pending interrupt
- **THEN** 返回 404 或友好的"没有待确认的操作"消息

### Requirement: 前端确认 UI
前端 SHALL 在收到 `interrupt` SSE 事件时:
- 停止等待更多事件
- 在聊天列表中渲染一个确认气泡(含 reason 文本 + 两个按钮)
- 用户点"确认" → 调用 `POST /api/chat/resume?sessionId=...&action=confirm`,以 EventSource 接收后续 SSE 流
- 用户点"取消" → 调用 `POST /api/chat/resume?sessionId=...&action=cancel`,显示取消文本
...
- **THEN** 前端调 `POST /api/chat/resume?action=confirm`,以 EventSource 接收 SSE
- **AND** 后续 chart + text 事件正常渲染
- **AND** 输入框恢复可用

#### Scenario: 用户点取消
- **WHEN** 用户点"取消"按钮
- **THEN** 前端调 `POST /api/chat/resume?action=cancel`
- **AND** 显示"已取消"文本
- **AND** 输入框恢复可用
