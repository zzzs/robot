...
系统 SHALL 提供 `POST /api/eval/run` 端点,触发 eval 运行并返回 JSON 报告。端点 SHOULD 支持 `?category=integrity` 查询参数过滤只跑某类用例。

#### Scenario: 运行全量 eval
- **WHEN** `POST /api/eval/run`(不带 category 参数)
- **THEN** 运行所有用例,返回完整报告

#### Scenario: 只跑离线用例
- **WHEN** `POST /api/eval/run?offline=true`
- **THEN** 只跑 `requiresNetwork: false` 的用例(快速验证,不依赖外部 API)

### Requirement: Eval 模块独立,不影响现有功能
eval 模块 MUST 不修改任何现有代码(chat / stock / news / orchestrator)。它只读 `ChatService.chat()` 接口,不直接修改 agent 行为。eval 模块 MUST 有自己的 Nest module + controller。

#### Scenario: eval 模块不干扰 chat 接口
- **WHEN** eval 端点被调用
- **THEN** chat 接口(`POST /api/chat` / `GET /api/chat/stream`)不受影响
- **AND** eval 使用独立的 sessionId 前缀(`eval-`),不污染正常对话历史
