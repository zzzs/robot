...
代码/文档知识库 RAG (Retrieval-Augmented Generation) 能力。索引代码(.ts/.tsx/.js/.jsx)与文档(.md/.mdx)到 pgvector,通过 `search_codebase` 工具暴露给 chat agent,支持代码实现、业务文档、设计决策等问答场景。索引基于 LlamaIndex.TS `CodeSplitter`(tree-sitter AST)按函数/类切代码 + `MarkdownNodeParser` 按标题切文档,存到 Supabase pgvector(512 维 GLM embedding-3)。检索侧经过三波迭代优化:Hybrid Search(向量+关键词加权)→ Query Rewriting + Rerank → SimilarityPostprocessor + AutoMergingRetriever + HyDE,精准度从 6/10 提升至 9.5/10。
