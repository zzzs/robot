...
Shared Postgres connection pool + migration runner + PostgresSaver singleton for LangGraph checkpoint persistence. Backed by cloud Postgres (Supabase by default — see `learn/postgres_runbook.md`). Falls back to in-memory when `DATABASE_URL` is unset (for dev without DB).

Implemented by:
- **PostgresModule** (`backend/src/postgres/postgres.module.ts`) — `@Global()` module
- **PostgresPoolService** (`postgres-pool.service.ts`) — single shared `Pool` instance with `isAvailable()` check
- **MigrationsTrackerService** (`migrations-tracker.service.ts`) — runs `.sql` files from `migrations/` in lexical order, tracks applied in `migrations` table; also calls `PostgresSaver.setup()` once globally
- **POSTGRES_SAVER** symbol — singleton provider; both `LangGraphOrchestrator` and `CreateAgentOrchestrator` inject the same instance

Migration files (`migrations/`):
- `001_init_messages.sql` — `messages` table (session_id, role, content_json JSONB, additional_kwargs_json JSONB, tool_calls_json JSONB)
- `002_news_vectors.sql` — `news_vectors` table (embedding vector(512)) + HNSW cosine index

Dual-mode behavior:
- `DATABASE_URL` set → Postgres (persistent, all 4 in-memory states replaced)
- `DATABASE_URL` empty → graceful fallback to `MemorySaver` / `MemoryVectorStore` / `InMemoryChatMessageHistory`
