...
Wrap the company's internal component registry (`pi.paas-test.cai-inc.com`) as an MCP server so any LLM client (Claude Desktop, Cursor, LangChain agent, etc.) can query components without writing custom HTTP wrappers. Implemented by:

- **MCP server**: `mcp-servers/cai-comp/` — standalone npm subpackage, stdio transport, exposes 2 tools (`get_comp_detail`, `list_comps`)
- **NestJS client**: `backend/src/cai-comp/` — `McpCaiCompClient` spawns the MCP server via `StdioClientTransport`, exposes `CAI_COMP_GET_DETAIL_TOOL` / `CAI_COMP_LIST_TOOL` providers
- **Tool wrappers**: `DynamicStructuredTool` instances registered in `CaiCompModule`, mounted on all 4 orchestrators (manual / langgraph / supervisor / create-agent)

Auth via 5 env vars (`CAI_COMP_UID` / `CAI_ATOM_TOKEN` / `CAI_SSO_TOKEN` / `CAI_CONGRESS` / `CAI_ONLINE_TICKET`) — cookies from the browser. Token expiry handled gracefully (tool returns `{status:'unauthorized'}` so the agent tells the user to refresh env vars).

See `learn/cai_comp_mcp.md` for the MCP protocol tutorial.
...
- **THEN** the tool SHALL return `{ status: 'unauthorized', hint: 'token 过期,检查 CAI_ATOM_TOKEN / CAI_SSO_TOKEN / CAI_CONGRESS env vars' }`
...
- **THEN** the client SHALL spawn `node ../mcp-servers/cai-comp/dist/index.js` as a stdio child process (relative path resolved from the backend's cwd)
