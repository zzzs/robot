...
基于 `langchain` 包 `createAgent` API 的预构建编排器,实现 `ChatOrchestratorInterface`,作为 LangGraph/Reflexion 之外的可选模式(`ORCHESTRATOR=create-agent`)。相比手写 StateGraph,适合简单 ReAct 场景(无条件 HITL、无副通道),代码量少。chart 副通道通过工具闭包实现,token 级流式通过 `streamMode: ['values', 'messages']` 提取 agent 节点 chunk。
